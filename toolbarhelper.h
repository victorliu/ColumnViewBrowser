...
public:
				   ...
				   DWORD dwComboStyle = CBS_DROPDOWNLIST)
	{
		...
		if (nDiff > 1)
		{
			...
			combo.MoveWindow(rectCombo.left, rc.top + (nDiff / 2), rectToolBar.Width(), rectCombo.Height());
		}else{
			toolbar.ScreenToClient(&rectCombo);
			combo.MoveWindow(rectCombo.left, rc.top, rectToolBar.Width(), rectCombo.Height());
		}
		return combo;
	}
